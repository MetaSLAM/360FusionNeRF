...
def load_st3d_data(baseDir='/home/jessie/datasets/st3d_rgbdxyz/nerf/03007_834036', stage=0):
    ...
    if stage > 0:
        ...
        for i, p in enumerate(cam_origin):
            dep = np.linalg.norm(coord - p, axis=-1)  # coord is a spherical projection, now from p depth is calculated
            ...
            if i<100:
                ...
                rgb_test = np.asarray(Image.open(os.path.join(baseDir, 'test', 'rgb_{}.png'.format(i-100))).convert('RGB'))
