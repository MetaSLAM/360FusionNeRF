...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        # print('size of rgb: ', rgb.shape)
        # print('size of target_rgb: ', target_rgb.shape)
        ...
        loss = img_loss + 0.1*depth_loss + grad_loss
        ...
        if i%args.i_print==0:
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}  Image Loss: {img_loss.item()}  Depth Loss: {depth_loss.item()}")
            with open(training_log, 'a') as f:
                f.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}  Image Loss: {img_loss.item()}  Depth Loss: {depth_loss.item()} \n")
