...
def render_images(rays, hw, render_kwargs, i_images, render_factor=0):
    ...
    for i in tqdm(i_images):
        rgb, _, _,  _ = render(H, W, rays=[rays_o[i*batch:(i+1)*batch], rays_d[i*batch:(i+1)*batch]], **render_kwargs)
...
def render(H, W, rays, near=0.0, far=2.0 ,use_viewdirs=True, **kwargs):
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk=1024*256, **kwargs)
    sh = rays_d.shape
    for k in all_ret:
...
def config_parser():
    ...
    parser.add_argument("--netchunk", type=int, default=1024*64*4, 
                        help='number of pts sent through network in parallel, decrease if running out of memory')
    parser.add_argument("--no_reload", action='store_true', 
                        help='do not reload weights from saved ckpt')
    parser.add_argument("--ft_path", type=str, default= None, 
                        help='specific weights npy file to reload for coarse network')
    # '/home/skulkarni/vit_loss/omni_nerf/logs/std3d_with_vit_v0/200000.tar'
    ...
    parser.add_argument("--i_clip_loss", type=int, default=100, 
                        help='frequency of clip loss calculation')
...
def train():
    ...
    clip_embed = get_embed_fn('clip_vit', -1, checkpoint=True, clip_cache_root=os.path.expanduser("~/.cache/clip"))

    print('clip embedding created')
    ...
    training_log = os.path.join(basedir, expname, 'training_log.txt')
    ...
    for i in trange(start, N_iters):
        ...
        # depth_loss
        if args.use_depth:
            # print('dep shape: ', dep.shape, 'target_depth shape: ', target_depth.shape)
            depth_loss = img2mse(dep.to(device), torch.tensor(target_depth).to(device))
            depth_loss = depth_loss/torch.var(dep.to(device))
        else:
        ...
        loss = img_loss + 0.0005*depth_loss + grad_loss
        ...
        if 'rgb0' in extras:
            ...
            if args.use_depth:
                ...
                depth_loss0 = torch.abs(extras['depth0'] - torch.tensor(target_depth)).mean()                
                loss = loss + 0.005*depth_loss0
            if args.use_gradient:
        ...
        if i%args.i_clip_loss==0 and i > 0:
            # rays_rgb_test = torch.reshape(rays_rgb_test, (2, 512, 1024, 3))

            i_consis = np.random.choice(10, size = 2, replace = False)            

            with torch.no_grad():
                rgbs = render_images([rays_o_test, rays_d_test], hw, render_kwargs_test, i_images= i_consis, render_factor=args.render_factor)

            rgbs_tensor = torch.tensor(rgbs)
            rgbs_resize = F.interpolate(torch.permute(rgbs_tensor, (0, 3, 1, 2)), size=(224, 224), mode='bicubic')

            rendered_embeddings = clip_embed(rgbs_resize)
            rendered_emb1 = rendered_embeddings[0][0]
            rendered_emb2 = rendered_embeddings[1][0]


            consistency_loss = -torch.cosine_similarity(rendered_emb1, rendered_emb2, dim=-1)
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            rays_rgb_test = torch.reshape(rays_rgb_test, (11, H, W, 3))
            ...
            # calculate MSE and PSNR for all test images
            gt_loss = img2mse(torch.tensor(rgbs[-1]), torch.tensor(rays_rgb_test[-1]))
            gt_psnr = mse2psnr(gt_loss)

            ssim_rgbs = torch.unsqueeze(torch.tensor(rgbs[-1]).permute(2, 0, 1), 0)
            ssim_test = torch.unsqueeze(rays_rgb_test[-1].clone().detach().permute(2, 0, 1), 0)

            ssim_metric = ssim(ssim_rgbs, ssim_test)
            try:
                lpips_metric = lpips(ssim_rgbs.to("cpu"), ssim_test.to("cpu"))
            except:
                print('Exception for LPIPS occured')
                lpips_metric = 0

            print('ground truth loss: {}, psnr: {}'.format(gt_loss, gt_psnr))
            with open(os.path.join(testsavedir, 'statistics.txt'), 'w') as f:
                f.write('loss: {}, psnr: {} \n'.format(gt_loss, gt_psnr))
                f.write('SSIM: {},  LPIPS VGG: {}'.format(ssim_metric, lpips_metric))
        ...
        if i%args.i_print==0:
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}  Image Loss: {img_loss.item()}  Consistency Loss: {consistency_loss.item()} Depth Loss: {depth_loss.item()}")
            with open(training_log, 'a') as f:
                f.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}  Image Loss: {img_loss.item()}  Consistency Loss: {consistency_loss.item()} Depth Loss: {depth_loss.item()} \n")
